...
import { getAccessToken, getRefreshToken, writeAuthTokens, writeCachedAuthUser } from '../features/auth/authStorage'
...
export async function refreshSession({ startupTrace = false, timeoutMs, maxRetries = 2 } = {}) {
  if (!refreshPromise) {
    ...
    refreshPromise = request('/auth/refresh', {
      ...
      .then((data) => {
        ...
        if (data?.user?.id) writeCachedAuthUser(data.user)
        return data
      })
      .finally(() => {
        refreshPromise = null
