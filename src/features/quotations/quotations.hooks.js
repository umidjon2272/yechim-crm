...
export function useQuotations(initialParams = {}) {
  const [params, setParams] = useState({ page: 1, pageSize: 10, search: '', status: '', ...initialParams })

  const { data, loading, error, refetch } = useAsync(
    () => quotationsService.list(params),
    [params.page, params.pageSize, params.search, params.status, params.dealId]
  )
...
export function useQuotation(id) {
  return useAsync(() => quotationsService.get(id), [id])
}
