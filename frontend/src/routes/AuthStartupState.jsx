...
export function AuthStartupState() {
  ...
  if (!slow && !isStartupError) return <div className="full-page-loading"><Spinner size="lg" /></div>
  ...
  return (
    <div className="full-page-loading startup-state">
      <div className="startup-state__card">
        ...
        {!isChecking && <button type="button" className="btn btn--secondary" onClick={() => { setSlow(false); refreshUser() }}>Qayta urinish</button>}
      </div>
    </div>
  )
}
